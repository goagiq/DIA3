...
class UnifiedTextAgent(BaseAgent):
    ...
    async def process_content(self, content: str, language: str = "en", options: dict = None) -> dict:
        """
        Process content - interface method for MCP server compatibility.
        ...
        Returns:
            Processing result
        """
        # Use the same implementation as process_text for compatibility
        if options is None:
            options = {}
        options["language"] = language
        return await self.process_text(content, options)
