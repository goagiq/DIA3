...
# Enhanced MCP Tool Detection Patterns for Multilingual Support
MCP_TOOL_DETECTION_PATTERNS = {
    "en": {
        # Core request phrases
        "bulk_import": [
            r"add\s+the\s+following",
            r"process\s+these",
            r"include\s+the\s+following",
            r"include\s+these",
            r"add\s+these\s+urls",
            r"process\s+these\s+urls",
            r"include\s+these\s+urls",
            r"add\s+the\s+following\s+urls",
            r"process\s+the\s+following\s+urls",
            r"include\s+the\s+following\s+urls",
        ],
        "url_processing": [
            r"process\s+@[^\s]+",
            r"add\s+@[^\s]+",
            r"include\s+@[^\s]+",
            r"process\s+the\s+following",
            r"@https?://",
            r"process\s+url",
            r"add\s+url",
            r"include\s+url",
        ...
        "database_storage": [
            r"to\s+vector\s+db",
            r"to\s+knowledge\s+graph",
            r"to\s+both\s+databases",
            r"to\s+vector\s+and\s+knowledge\s+graph",
            r"store\s+in\s+vector",
            r"store\s+in\s+knowledge\s+graph",
            r"store\s+in\s+both",
            r"store\s+in\s+database",
            r"save\s+to\s+database",
            r"add\s+to\s+database",
            r"insert\s+into\s+database",
            r"database\s+storage",
        ],
        "mcp_tool_specific": [
            r"mcp\s+tool",
            r"mcp\s+server",
            r"process\s+content",
            r"unified\s+processing",
            r"content\s+processing",
        ],
        "content_sources": [
            r"openlibrary",
            r"ctext\.org",
        ...
        "content_analysis": [
            ...
            r"generate\s+knowledge\s+graph",
            r"create\s+vector\s+embeddings",
            r"semantic\s+analysis",
        ],
        "multi_step_processing": [
            r"process\s+and\s+store",
            r"analyze\s+and\s+save",
            r"extract\s+and\s+store",
            r"process\s+then\s+store",
            r"analyze\s+then\s+save"
        ],
        "processing_keywords": [
    ...
    "zh": {
        # Chinese MCP tool detection patterns
        "bulk_import": [
            r"添加以下",
            r"处理这些",
            r"包含以下",
        ...
        "processing_keywords": [
            '处理', '添加', '包含', '存储', '保存', '分析', '提取',
            '向量', '知识', '图谱', '数据库', '网址', '内容'
        ]
    },
    
    "ru": {
        # Russian MCP tool detection patterns
        "bulk_import": [
            r"добавить\s+следующие",
            r"обработать\s+эти",
            r"включить\s+следующие",
            r"включить\s+эти",
            r"добавить\s+эти\s+ссылки",
            r"обработать\s+эти\s+ссылки",
            r"включить\s+эти\s+ссылки",
            r"добавить\s+следующие\s+ссылки",
            r"обработать\s+следующие\s+ссылки",
            r"включить\s+следующие\s+ссылки",
        ],
        "url_processing": [
            r"обработать\s*@[^\s]+",
            r"добавить\s*@[^\s]+",
            r"включить\s*@[^\s]+",
            r"обработать\s+следующие",
            r"@https?://",
            r"обработать\s+ссылку",
            r"добавить\s+ссылку",
            r"включить\s+ссылку",
        ],
        "database_storage": [
            r"в\s+векторную\s+базу\s+данных",
            r"в\s+граф\s+знаний",
            r"в\s+оба\s+базы\s+данных",
            r"в\s+векторную\s+и\s+граф\s+знаний",
            r"сохранить\s+в\s+векторную",
            r"сохранить\s+в\s+граф\s+знаний",
            r"сохранить\s+в\s+оба",
            r"сохранить\s+в\s+базу\s+данных",
            r"сохранить\s+в\s+базу",
            r"добавить\s+в\s+базу\s+данных",
            r"вставить\s+в\s+базу\s+данных",
            r"хранение\s+в\s+базе\s+данных",
        ],
        "mcp_tool_specific": [
            r"mcp\s+инструмент",
            r"mcp\s+сервер",
            r"обработать\s+содержимое",
            r"унифицированная\s+обработка",
            r"обработка\s+содержимого",
        ],
        "content_sources": [
            r"открытая\s+библиотека",
            r"китайский\s+текстовый\s+проект",
            r"библиотечный\s+контент",
            r"книжный\s+контент",
            r"обработка\s+документов",
        ],
        "batch_processing": [
            r"пакетная\s+обработка",
            r"массовый\s+импорт",
            r"несколько\s+ссылок",
            r"множественные\s+ссылки",
            r"список\s+ссылок",
        ],
        "content_analysis": [
            r"анализировать\s+содержимое",
            r"извлекать\s+сущности",
            r"генерировать\s+граф\s+знаний",
            r"создавать\s+векторные\s+вложения",
            r"семантический\s+анализ",
        ],
        "multi_step_processing": [
            r"обработать\s+и\s+сохранить",
            r"анализировать\s+и\s+сохранить",
            r"извлечь\s+и\s+сохранить",
            r"обработать\s+затем\s+сохранить",
            r"анализировать\s+затем\s+сохранить"
        ],
        "processing_keywords": [
            'обработать', 'добавить', 'включить', 'сохранить', 'анализировать', 'извлечь',
            'вектор', 'знания', 'граф', 'база', 'ссылка', 'содержимое'
        ]
    }
}
...
def get_mcp_tool_detection_patterns(language: str) -> Dict[str, List[str]]:
    """Get MCP tool detection patterns for a specific language."""
    return MCP_TOOL_DETECTION_PATTERNS.get(language, MCP_TOOL_DETECTION_PATTERNS["en"])


def get_all_mcp_detection_patterns(language: str) -> List[str]:
    """Get all MCP detection patterns for a specific language as a flat list."""
    patterns = get_mcp_tool_detection_patterns(language)
    all_patterns = []
    
    for category, pattern_list in patterns.items():
        if category != "processing_keywords":  # Skip keywords, they're handled separately
            all_patterns.extend(pattern_list)
    
    return all_patterns


def get_mcp_processing_keywords(language: str) -> List[str]:
    """Get processing keywords for a specific language."""
    patterns = get_mcp_tool_detection_patterns(language)
    return patterns.get("processing_keywords", patterns.get("en", {}).get("processing_keywords", []))


def detect_language_for_mcp_detection(text: str) -> str:
    """Detect the primary language of the text for MCP tool detection."""
