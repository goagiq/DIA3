#!/usr/bin/env python3
"""
PDF Generation Service for DIA3
...
class PDFGenerationService:
    """Service for generating PDFs from markdown content with mermaid diagrams."""
    ...
    async def generate_mermaid_image(self, mermaid_code: str, diagram_type: str = "flowchart") -> Optional[bytes]:
    ...
    def convert_mermaid_to_images(self, markdown_content: str, output_dir: Path) -> str:
        ...
        def replace_mermaid(match):
            ...
            # Generate image (synchronous for now)
            try:
                # Create a new event loop for this operation
                new_loop = asyncio.new_event_loop()
                asyncio.set_event_loop(new_loop)
                image_data = new_loop.run_until_complete(
                    self.generate_mermaid_image(diagram_content, diagram_type.lower())
                )
                new_loop.close()
            except RuntimeError:
                ...
                try:
                    ...
                    image_data = asyncio.get_event_loop().run_until_complete(task)
                except Exception as e:
                    logger.warning(f"Could not generate mermaid image: {e}")
                    image_data = None
