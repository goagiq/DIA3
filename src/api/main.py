...
try:
    from src.api.strategic_deception_routes import router as strategic_deception_router, set_orchestrator
    STRATEGIC_DECEPTION_AVAILABLE = True
except ImportError as e:
    logger.warning(f"Strategic deception routes not available: {e}")
    STRATEGIC_DECEPTION_AVAILABLE = False

# Import escalation analysis routes
try:
    from src.api.escalation_analysis_routes import router as escalation_analysis_router
    ESCALATION_ANALYSIS_AVAILABLE = True
except ImportError as e:
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Set orchestrator reference for strategic deception routes if available
...
if STRATEGIC_DECEPTION_AVAILABLE:
    app.include_router(strategic_deception_router)
    logger.info("✅ Strategic deception monitoring routes included")
else:
    logger.warning("⚠️ Strategic deception monitoring routes not available")

# Include escalation analysis routes if available
if ESCALATION_ANALYSIS_AVAILABLE:
    app.include_router(escalation_analysis_router)
...
class ScenarioAnalysisRequest(BaseModel):
    base_data: str  # JSON string
    scenarios: str  # JSON string
    target_variable: str
...
class MarketDataRequest(BaseModel):
    market_sector: str
    data_types: List[str] = ["sentiment", "trends", "news"]
    time_range: str = "30d"
    include_competitors: bool = True


@app.post("/business/intelligence-analysis")
async def business_intelligence_analysis(request: BusinessIntelligenceRequest):
    """Analyze business intelligence using MCP tools."""
    try:
        ...
        # Use the business intelligence analysis tool
        result = await agent.create_business_intelligence_report(
            data_sources=[request.content],
            report_scope=request.analysis_type,
        ...
        logger.error(f"Business intelligence analysis error: {str(e)}")
...
async def strategic_assessment_endpoint(request: StrategicAssessmentRequest):
    ...
    try:
        ...
        # Initialize the Art of War deception agent
        agent = ArtOfWarDeceptionAgent()
        
        # Use the strategic assessment tool
        result = await agent.analyze_deception_techniques(
            analysis_type=request.analysis_type,
...
@app.post("/strategic/art-of-war-deception")
async def art_of_war_deception_endpoint(request: StrategicAssessmentRequest):
    ...
    try:
        ...
        # Use the Art of War deception analysis tool
        result = await agent.analyze_deception_techniques(
            analysis_type=request.analysis_type,
            focus_areas=request.focus_areas or ["strategic_deception", "information_warfare"],
            include_modern_applications=request.include_modern_applications,
        ...
        return {
            ...
            "modern_applications": result.get("modern_applications", {}),
            "ethical_considerations": result.get("ethical_considerations", {}),
            "analysis_type": request.analysis_type
        }
        
    except Exception as e:
        logger.error(f"Art of War deception analysis error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Art of War deception analysis failed: {str(e)}")


@app.post("/advanced-analytics/scenario")
async def scenario_analysis_endpoint(request: ScenarioAnalysisRequest):
    """Perform scenario analysis."""
    try:
        from src.agents.scenario_analysis_agent import ScenarioAnalysisAgent
        import json
        ...
        # Parse JSON data
        base_data = json.loads(request.base_data)
        scenarios = json.loads(request.scenarios)
        
        # Use the scenario analysis tool
        from src.core.models import AnalysisRequest, DataType
...
@app.post("/integrate/market-data")
async def market_data_analysis_endpoint(request: MarketDataRequest):
    """Analyze market data."""
    try:
        from src.agents.market_data_agent import MarketDataManager
        
        # Initialize the market data manager
        agent = MarketDataManager()
        
        # Use the market data analysis tool
        ...
        logger.error(f"Market data analysis error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Market data analysis failed: {str(e)}")
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "art_of_war_deception": "/strategic/art-of-war-deception",
            "comprehensive_strategic_analysis": "/strategic/comprehensive-analysis",
            "strategic_status": "/strategic/status"
        }
    }

@app.post("/business/strategic-position-analysis")
async def business_strategic_position_analysis(request: TextRequest):
    """
    Analyze business strategic position in current market conditions.
    
    This endpoint provides comprehensive business strategic analysis using:
    - Art of War principles and scenario analysis
    - Multi-domain strategic analysis framework
    - Current market conditions assessment
    - Competitive landscape analysis
    - Strategic recommendations and implementation roadmap
    """
    try:
        logger.info("Starting business strategic position analysis")
        
        # Initialize strategic analysis components
        analysis_result = {
            "success": True,
            "analysis_type": "business_strategic_position",
            "timestamp": time.time(),
            "components": []
        }
        
        # Component 1: Art of War Scenario Analysis
        try:
            from Test.art_of_war_scenario_analysis import ArtOfWarScenarioAnalysis
            
            art_of_war_analyzer = ArtOfWarScenarioAnalysis()
            art_of_war_results = art_of_war_analyzer.run_complete_analysis()
            
            analysis_result["components"].append({
                "name": "art_of_war_scenario_analysis",
                "status": "success",
                "data": art_of_war_results
            })
            
            logger.info("✅ Art of War scenario analysis completed")
        except Exception as e:
            logger.error(f"❌ Art of War analysis failed: {e}")
            analysis_result["components"].append({
                "name": "art_of_war_scenario_analysis",
                "status": "error",
                "error": str(e)
        ...
        # Component 2: Multi-Domain Strategic Analysis
        try:
            from src.core.multi_domain_strategic_analyzer import (
                MultiDomainStrategicAnalyzer, DomainType, AnalysisType
            )
            
            multi_domain_analyzer = MultiDomainStrategicAnalyzer()
            multi_domain_results = await multi_domain_analyzer.analyze_strategic_position(
                content=request.content,
            ...
            analysis_result["components"].append({
                "name": "multi_domain_strategic_analysis",
                "status": "success",
                "data": multi_domain_results
            })
            
            logger.info("✅ Multi-domain strategic analysis completed")
        except Exception as e:
        ...
        # Component 3: Market Data Analysis
        try:
            from src.agents.market_data_agent import MarketDataManager
            
            market_manager = MarketDataManager()
            market_results = await market_manager.analyze_market_data(
        ...
        # Component 4: Business Intelligence Analysis
        try:
            from src.agents.business_intelligence_agent import BusinessIntelligenceAgent
            
            bi_agent = BusinessIntelligenceAgent()
            bi_results = await bi_agent._generate_comprehensive_analysis(request.content)
        ...
        successful_components = [
            comp for comp in analysis_result["components"] 
        ...
        if successful_components:
            ...
            analysis_result["summary"] = {
                ...
                "recommendations": ["Review system configuration and dependencies"]
            }
        
        logger.info("✅ Business strategic position analysis completed")
        return analysis_result
        
    except Exception as e:
        logger.error(f"❌ Business strategic position analysis failed: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Business strategic position analysis failed: {str(e)}"
        )
...
class PatternAnalysisRequest(BaseModel):
    ...
    analysis_type: str = "comprehensive"  # comprehensive, strategic, cultural, pattern
    use_mcp_tools: bool = True
    document_path: Optional[str] = None  # For single document analysis


@app.post("/pattern-analysis/comprehensive")
async def run_comprehensive_pattern_analysis(request: PatternAnalysisRequest):
    ...
    try:
        ...
        results = await agent.run_comprehensive_analysis(use_mcp_tools=request.use_mcp_tools)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Comprehensive pattern analysis failed: {str(e)}"
        )


@app.post("/pattern-analysis/single-document")
async def analyze_single_document_pattern(request: PatternAnalysisRequest):
    """Analyze a single document for patterns."""
    try:
        if not request.document_path:
            raise HTTPException(
                status_code=400,
                detail="document_path is required for single document analysis"
            )
        
        from src.agents.pattern_analysis_agent import PatternAnalysisAgent
        
        agent = PatternAnalysisAgent()
        results = await agent.analyze_single_document(
        ...
        logger.error(f"❌ Single document pattern analysis failed: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Single document pattern analysis failed: {str(e)}"
        )


@app.get("/pattern-analysis/health")
async def pattern_analysis_health():
    ...
    try:
        ...
        logger.error(f"❌ Pattern analysis health check failed: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Pattern analysis health check failed: {str(e)}"
        )


# Classical Chinese HUMINT Analysis Endpoints
class ClassicalChineseHUMINTRequest(BaseModel):
    """Request model for Classical Chinese HUMINT analysis."""
    content: str
    language: str = "zh"
    source_info: Optional[Dict[str, Any]] = None
    analysis_type: str = "comprehensive"  # comprehensive, strategic, cultural, deception


@app.post("/classical-chinese-humint/analyze")
async def analyze_classical_chinese_humint(request: ClassicalChineseHUMINTRequest):
    ...
    try:
        ...
        analysis_request = AnalysisRequest(
            ...
            data_type=DataType.TEXT,
            language=request.language,
            metadata=request.source_info or {}
        )
        ...
        logger.error(f"❌ Classical Chinese HUMINT analysis failed: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Classical Chinese HUMINT analysis failed: {str(e)}"
        )


@app.post("/classical-chinese-humint/strategic-deception")
async def analyze_strategic_deception(request: ClassicalChineseHUMINTRequest):
    """Analyze Classical Chinese content for strategic deception patterns."""
    try:
        from src.agents.classical_chinese_humint_agent import ClassicalChineseHUMINTAnalysisAgent
        ...
        analysis_request = AnalysisRequest(
            ...
            metadata={
                **(request.source_info or {}),
                "analysis_focus": "strategic_deception"
            }
        )
        ...
        # Extract strategic deception specific results
        classical_analysis = result.results.get("classical_chinese_analysis", {})
        deception_indicators = [
            ind for ind in classical_analysis.get("strategic_indicators", [])
            if ind.get("type") == "deception_principle"
        ]
        
        return {
            "status": "success",
            "analysis_type": "strategic_deception",
            "framework": "Classical Chinese Strategic Deception Analysis",
            "deception_indicators": deception_indicators,
            "deception_risk": result.results.get("humint_implications", {}).get("deception_risk", {}),
        ...
        logger.error(f"❌ Strategic deception analysis failed: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Strategic deception analysis failed: {str(e)}"
        )


@app.get("/classical-chinese-humint/health")
async def classical_chinese_humint_health():
    """Health check for Classical Chinese HUMINT analysis service."""
    try:
        from src.agents.classical_chinese_humint_agent import ClassicalChineseHUMINTAnalysisAgent
        ...
        return {
            "status": "healthy",
            "service": "classical_chinese_humint_analysis",
            "framework": "Classical Chinese HUMINT Analysis Framework",
            "capabilities": [
            ...
            "strategic_indicators": {
                "deception_principles": 12,
                "strategic_concepts": 7,
                "cultural_values": 16
            },
            "timestamp": datetime.now().isoformat()
        }
        ...
        logger.error(f"❌ Classical Chinese HUMINT health check failed: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Classical Chinese HUMINT health check failed: {str(e)}"
        )
