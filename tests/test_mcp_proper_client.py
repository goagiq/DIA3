#!/usr/bin/env python3
"""
Test MCP server using the proper MCP client approach with streamable HTTP.
...
def test_mcp_proper_client():
    """Test MCP server using the proper client approach."""
    print("=== TESTING MCP SERVER WITH PROPER CLIENT ===")
    ...
    try:
        ...
        # Test the client connection using the proper approach
        try:
            # Create the MCP client using the proper approach
...
def main():
    """Main test function."""
    print("🚀 Starting MCP Proper Client Test")
    print("=" * 60)
    
    # Test results
    results = {}
    
    # Test 1: MCP Server with proper client
    results['mcp_proper_client'] = test_mcp_proper_client()
    ...
    if all_passed:
        ...
        print("The 'Missing session ID' issue has been resolved using proper MCP client.")
    else:
